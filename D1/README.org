...
void parse_file(std::string_view filepath) {
    ...
    for (std::string line; std::getline(file, line); ) {
        ...
        int value = 0;
        auto [ptr, ec] = std::from_chars(line.data() + 1, line.data() + line.size(), value);
...
void fright(int &dial, int r){
    dial = floor_mod( (dial + r), 99 );
}

void fleft(int &dial, int l){
    dial = floor_mod((dial - l), 99);
}
...
/***/
    ...
    std::cout << dial_test << " dial_test 50\n";
    
    fright(dial_test, 10);


    std::cout << dial_test << " dial_test R10\n";
